...
public class GameManager : MonoBehaviour
{
    ...
    [SerializeField] private Text killText;                       //킬 수
    public  int Totalkill = 0;                                    //총 킬 수
    public static GameManager instance;                           //Singleton : GameManager(게임 전체 관리) Class
                                                                  // - 다른 곳에서 게임 매니저에 쉽게 접근하기 위해 싱글턴을 막든다.  
                                                                  //1. public 이라고 선언된 변수나 함수에 쉽게 접근 가능하게
                                                                  //2. 무분별한 개체 생성 막음

    void Start()
    {
        instance = this;             //GameManager Class script (이 스크립트 자기 자신) 
        killText = GameObject.Find("Canvas_UI").transform.GetChild(1).GetChild(0).GetComponent<Text>();
    ...
    //public
    public void KillScore(int _score)        //킬 수 관리 
    {
        Totalkill += _score;
        killText.text = "Kill : " + "<color=#ff0000>" + Totalkill.ToString() + "</color>" ;  //HTML
    }
